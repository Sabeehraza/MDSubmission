function AddEllipticalAtomicArray(Width, Hight, X0, Y0, VX0, VY0, InitDist, Temp, Type)
global C
...
L = (2*Width - 1) * AtomSpacing;
W = (2*Hight - 1) * AtomSpacing;

% attom array
xp(1, :) = linspace(-L/2, L/2, 2*Width);
yp(1, :) = linspace(-W/2, W/2, 2*Hight);


numAtoms = 0;
for i = 1:2*Width
    for j = 1:2*Hight
        if (xp(i)/(Width*AtomSpacing))^2 + (yp(j)/(Hight*AtomSpacing))^2 <= 1
            numAtoms = numAtoms+1;
